...
#         # conda activate gpen
#         # cd ../GPEN
#         # python demo.py --task FaceEnhancement --model GPEN-BFR-256 --in_size 256 --channel_multiplier 1 --narrow 0.5 --use_sr --sr_scale 4 --use_cuda --save_face --indir $shot_dir --outdir SR_${shot_dir}
#         # cd $root_dir
#         # conda deactivate
#         # ffmpeg -y -r 25 -i SR_${shot_dir}/%05d_GPEN.jpg -vf fps=25 ${res_dir}/SR_mute_${vid}
#         # ffmpeg -y -i ${res_dir}/SR_mute_${vid} -i $driving_video -map 0:v:0 -map 1:a:0 ${res_dir}/SR_${vid}
#         # rm -rf $shot_dir
#         # rm -rf SR_$shot_dir
#         # rm -rf ${res_dir}/SR_mute_${vid}

#         # ffmpeg -i source_image -i driving_video -i ${res_dir}/SR_${vid} -hstack=inputs=3 ${res_dir}/compare_${vid}
