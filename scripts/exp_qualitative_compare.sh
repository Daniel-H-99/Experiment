...
ours_dir=/home/server25/minyeong_workspace/Experiment/qualitative/Ours/202205180427
rhs_dir=/home/server25/minyeong_workspace/Experiment/qualitative/NFv2v/202205171758
...
checkpoint="/home/server25/minyeong_workspace/BFv2v/ckpt/00000034-checkpoint.pth.tar"
...
do
    ...
    # python demo.py --config config/vox-256-renderer_v3.yaml --checkpoint "${checkpoint}" --source_image $source_image --checkpoint_headmodel $checkpoint_headmodel --result_dir $res_dir --result_vid mute_$vid --driven_dir log_headmodel/v4.2 --driving_video $driving_video
    ...
    ours=${ours_dir}/SR_${vid}
    rhs=${rhs_dir}/SR_${vid}
    ffmpeg -i $driving_video -i $source_image -i $ours -i $rhs -filter_complex \
    "[0:v]scale=-1:256,crop=trunc(iw/2)*2:trunc(ih/2)*2[v1];\
    ...
    [v1][v2][v3][v4]hstack=inputs=4[v]" -map "[v]" -map 0:a ${res_dir}/compare_${vid}
